...
namespace UnitTestProject1
{
    ...
    public class UnitTest1<TTree> where TTree : ITree, new()
    {
        ITree lst = new TTree();
        ...
        {
            lst.Init(input);
            var ex = Assert.Throws<EmptyTreeEx>(() => lst.DelLeft(val));
        ...
        {
            lst.Init(input);
            var ex = Assert.Throws<ValueNotFoundEx>(() => lst.DelLeft(val));
